from fastapi.testclient import TestClient
from sqlalchemy import create_engine
from sqlalchemy.orm import sessionmaker
from main import app
from shared.database import Base
from shared.dependencies import get_db

# Essencial para melhorar a qualidade do código
# Era possível começar a aplicação escrevendo os testes
client = TestClient(app)

# Os testes apontam para um banco de dados local
# Esse arquivo vai conter os dados de teste
# Teste de banco de dados
SQLALCHEMY_DATABASE_URL = "sqlite:///./test.db"

# Cria uma nova engine
engine = create_engine(
    SQLALCHEMY_DATABASE_URL, 
    connect_args={"check_same_thread": False}
)

TestingSessionLocal = sessionmaker(
    autocommit=False,
    autoflush=False,
    bind=engine
)

def override_get_db():
    db = TestingSessionLocal()
    try:
        yield db
    finally:
        db.close()

app.dependency_overrides[get_db] = override_get_db

# Os testes a seguir são integrados, pois vão do controller até o banco de dados
# No caso dos testes, é interessante que o banco seja reiniciado a cada teste
def test_deve_listar_fornecedor_cliente():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    client.post(
        "/fornecedor-cliente/",
        json={'nome': 'CPFL'}
    )

    client.post(
        "/fornecedor-cliente/",
        json={'nome': 'Sanasa'}
    )

    response = client.get("/fornecedor-cliente/")
    assert response.status_code == 200
    assert response.json() == [
        {'id': 1, 'nome': 'CPFL'},
        {'id': 2, 'nome': 'Sanasa'}
    ]

def test_deve_pegar_por_id():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response = client.post(
        "/fornecedor-cliente/",
        json={'nome': 'CPFL'}
    )

    id_do_fornecedor_cliente = response.json()['id']

    response_get = client.get(
        f"/fornecedor-cliente/{id_do_fornecedor_cliente}"
    )

    assert response_get.status_code == 200
    assert response_get.json()['nome'] == "CPFL"

    id_do_fornecedor_cliente = response.json()['id']

    response_get = client.get(
        # Padrão do método PUT na API REST --> acrescentar id na URL
        f"/fornecedor-cliente/{id_do_fornecedor_cliente}"
    )

    assert response_get.status_code == 200
    assert response_get.json()['nome'] == "CPFL"

def test_deve_retornar_nao_encontrado_para_id_inexistente():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response = client.post(
        "/fornecedor-cliente/",
        json={'nome': 'CPFL'}
    )

    response_get = client.get(
        # Padrão do método PUT na API REST --> acrescentar id na URL
        f"/fornecedor-cliente/100"
    )

    assert response_get.status_code == 404

def test_deve_criar_fornecedor_cliente():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    novo_fornecedor_cliente = {
        "nome": "Extra Supermercado"
    }

    # Criar uma cópia do dicionário para evitar mutação
    novo_fornecedor_client_copy = novo_fornecedor_cliente.copy()

    # Temporariamente (enquanto não está automatizado),
    # Adicionar id manualmente
    novo_fornecedor_client_copy["id"] = 1

    response = client.post(
        "/fornecedor-cliente/",
        json=novo_fornecedor_client_copy
    )

    assert response.status_code == 201
    assert response.json() == novo_fornecedor_client_copy

def test_deve_atualizar_fornecedor_cliente():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response = client.post(
        "/fornecedor-cliente/",
        json={"nome": "Extra Supermercado"}
    )

    id_do_fornecedor_cliente = response.json()['id']

    response_put = client.put(
        # Padrão do método PUT na API REST --> acrescentar id na URL
        f"/fornecedor-cliente/{id_do_fornecedor_cliente}",
        json={"nome": "Giga Extra"}
    )

    assert response_put.status_code == 200
    assert response_put.json()['nome'] == "Giga Extra"


def test_deve_retornar_nao_encontrado_para_id_inexistente_na_atualizacao():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response_put = client.put(
        f"/fornecedor-cliente/100",
        json={"nome": "Giga Extra"}
    )

    assert response_put.status_code == 404

def test_deve_remover_fornecedor_cliente():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response = client.post(
        "/fornecedor-cliente/",
        json={"nome": "Adde Sistemas"}
    )

    id_do_fornecedor_cliente = response.json()['id']

    response_delete = client.delete(
        # Padrão do método DELETE na API REST --> acrescentar id na URL, não precisa de body
        f"/fornecedor-cliente/{id_do_fornecedor_cliente}"
    )

    assert response_delete.status_code == 204

    response_get_all = client.get("/fornecedor-cliente/")
    assert len(response_get_all.json()) == 0

def test_deve_retornar_nao_encontrado_para_id_inexistente_na_remocao():
    Base.metadata.dropall(bind=engine)
    Base.metadata.createall(bind=engine)

    response_delete = client.delete(
        f"/fornecedor-cliente/100"
    )

    assert response_delete.status_code == 404

def test_deve_retornar_erro_quando_o_nome_nao_estiver_dentro_dos_limites():
    response = client.post(
        "/fornecedor-cliente/",
        json={"nome": "A"}
    )

    assert response.status_code == 422
    assert response.json()['detail'][0]['loc'] == ["body","nome"]

    response = client.post(
        "/fornecedor-cliente/",
        json={"nome": ""}
    )
    
    assert response.status_code == 422
    assert response.json()['detail'][0]['loc'] == ["body","nome"]

