...
def listar_fornecedor_cliente(db: Session = Depends(get_db)):
    return db.query(FornecedorCliente).order_by(FornecedorCliente.id).all()
...
@router.put("/{id_do_fornecedor_cliente}", response_model=FornecedorClienteResponse)
...
def busca_fornecedor_cliente_por_id(
    ...
    fornecedor_cliente = db.get(FornecedorCliente, id_do_fornecedor_cliente)
